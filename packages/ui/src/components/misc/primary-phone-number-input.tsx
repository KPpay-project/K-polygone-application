...
export const PrimaryPhoneNumberInput: FC<PrimaryPhoneNumberInputProps> = ({
  ...
  const effectiveDefault = country || userCountry?.toLowerCase() || 'ng';

  const [validationError, setValidationError] = useState<string>('');
  const [isValid, setIsValid] = useState<boolean>(true);
  const [selectedCountry, setSelectedCountry] = useState<string>(effectiveDefault);
  ...
  const validatePhoneNumber = (phoneNumber: string, countryCode: string): boolean => {
    ...
    try {
      ...
      if (
        phoneNumberObj.getType() &&
        !['MOBILE', 'FIXED_LINE_OR_MOBILE'].includes(phoneNumberObj.getType() || '')
      ) {
