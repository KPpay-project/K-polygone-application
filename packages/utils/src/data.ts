  ...
  const tableData = data.map((item: any) => {
    ...
    fields.forEach((field: string | ((item: T) => any)) => {
      if (field === "S/N") {
        ...
        value = getMultiPropertyByPath<string>(item, path).values?.join(
          " ",
        ) as string;
...
export const gridCols = (widthString: string): number[] => {
  ...
  const totalValue = fractions.reduce(
    (accumulator, currentValue) => accumulator + currentValue,
    0,
  );
  const percentages = fractions.map(
    (fraction) => (fraction / totalValue) * (100 / 1),
  );
  return percentages;
};
