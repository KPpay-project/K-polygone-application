...
export const CountrySelector = forwardRef<HTMLButtonElement, CountrySelectorProps>(
  (
    ...
    const getCountryByCode = useCallback(
      (code?: string) => customCountries.find((c) => c.code === code),
      [customCountries]
    );
    ...
    useEffect(() => {
      ...
      if (country && country.code !== selectedCountry?.code) {
        ...
        onValueChange?.(customCountries[0].code, customCountries[0]);
      }
    }, [value, customCountries, getCountryByCode, onValueChange, selectedCountry?.code]);
          ...
          <SelectValue placeholder={placeholder}>
            {selectedCountry && (
              <div className="flex items-center gap-2">
                ...
                {showName && <span className="text-sm">{t(`countries.${selectedCountry.code}`)}</span>}
          ...
          {filteredCountries.map((country) => (
            <SelectItem key={country.code} value={country.code}>
              <div className="flex items-center gap-2">
                ...
                {showName && <span className="text-sm">{t(`countries.${country.code}`)}</span>}
