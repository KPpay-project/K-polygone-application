...
import { SidebarItemProps, useStaticMenuItems } from './sidebar-menu';
          ...
          {(item?.subItems || []).map((subItem, index) => {
            ...
            return (
              <li key={index} className="relative pl-6 overflow-hidden">
                ...
                <Link
                  to={subItem.href}
                  className={`block text-[13px] font-normal rounded-lg transition-all duration-300 ease-in-out px-3 py-2 ${
...
export const Sidebar: FC<SidebarProps> = ({ isOpen = true, onClose }) => {
  ...
  const menuItems = useStaticMenuItems();
