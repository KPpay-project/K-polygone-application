...
const identityDocumentSchema = z
  .object({
    ...
    dateOfIssue: z
      ...
      .refine((val) => val <= new Date(), { message: 'Date of issue cannot be in the future' }),
    expiryDate: z
      .date()
      .refine((val) => !!val, { message: 'Expiry date is required' })
      .refine((val) => val > new Date(), { message: 'Expiry date must be in the future' }),
    issuingAuthority: z
      .string()
      ...
      .regex(/^[A-Za-z\s\-',.]+$/, {
        message: 'Issuing authority can only contain letters, spaces and basic punctuation'
      }),
    identityDocument: z
      .any()
  ...
  .refine(
    ...
    {
      message: 'Document number format is invalid for the selected document type',
      path: ['documentNumber']
    }
  )
  .refine(
    (data) => {
      if (!data.documentType || !data.documentNumber || data.documentType !== 'nin') return true;
      return /^\d{11}$/.test(data.documentNumber);
    },
    {
  ...
  .refine(
    (data) => {
      if (!data.documentType || !data.documentNumber || data.documentType !== 'passport') return true;
      return /^[A-Z]{1,2}\d{6,8}$/i.test(data.documentNumber);
    },
    {
      message: 'Passport number must be 1-2 letters followed by 6-8 digits (e.g., A1234567)',
      path: ['documentNumber']
    }
  )
  .refine(
    (data) => {
      if (!data.documentType || !data.documentNumber || data.documentType !== 'drivers_license') return true;
      return /^[A-Z0-9]{8,15}$/i.test(data.documentNumber);
    },
    {
  ...
  .refine(
    (data) => {
      if (!data.documentType || !data.documentNumber || data.documentType !== 'national_id') return true;
      return /^\d{9,12}$/.test(data.documentNumber);
    },
    {
