...
import { FLUTTERWAVE_TEST_KEY } from '@/constant';
...
export class FlutterwaveBankService {
  static async fetchBanks(): Promise<FlutterwaveBank[]> {
    try {
      const response = await flutterwaveAxiosInstance.get(FLUTTERWAVE_API.BANKS);
  ...
  static async resolveBankAccount(accountNumber: string, bankCode: string): Promise<FlutterwaveAccountDetails | null> {
    try {
      const response = await flutterwaveAxiosInstance.post(FLUTTERWAVE_API.RESOLVE_ACCOUNT, {
