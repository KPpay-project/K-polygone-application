...
interface UseUnifiedBanksResult {
  ...
  loading: boolean;
  error: Error | null;
  resolveBankAccount: (accountNumber: string, bankCode: string) => Promise<UnifiedAccountDetails | null>;
  refreshBanks: () => Promise<void>;
}

export function useUnifiedBanks(provider: BankProvider, country?: string): UseUnifiedBanksResult {
  const [banks, setBanks] = useState<UnifiedBank[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);

  const bankService = useMemo(() => new UnifiedBankService({ provider, country }), [provider, country]);
