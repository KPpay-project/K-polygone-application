...
import { safeGraphQLOperation, type SafeOptions, type SafeResult } from '@/lib/graphql/wrapper';
import {
...
export function useSafeMutation<TData = any, TVars extends OperationVariables = OperationVariables, TPayload = any>(
  ...
  return [mutateSafe, { loading, data, error }];
}

type UseSafeQueryOptions<TData, TVars> = QueryHookOptions<TData, TVars> & {
  payloadPath?: string;
