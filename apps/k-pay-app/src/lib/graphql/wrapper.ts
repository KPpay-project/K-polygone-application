...
> extends MutationFunctionOptions<any, TVars> {
  payloadPath?: string;
...
export async function safeGraphQLOperation<
  TData,
  TVars = OperationVariables,
  TPayload = any,
>(
  ...
  try {
    ...
    if (options?.requireSuccess) {
      ...
      if (success === false) {
        const message: string =
          (payload as any)?.message || ErrorMessages[ErrorCode.UNKNOWN];
        const code =
    ...
    const normalized =
      apolloErr?.graphQLErrors || apolloErr?.networkError
        ...
        : new ValidationError(
            (e as Error)?.message || ErrorMessages[ErrorCode.UNKNOWN],
