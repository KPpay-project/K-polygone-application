...
export function useCreateTicketMessage() {
  ...
  const createTicketMessage = async (input: {
    ticketId: string;
    message: string;
  }) => {
...
export function useCreateTicket() {
  ...
  > => {
    return safeGraphQLOperation(mutate, {
      ...
      friendlyMessages: {
        VALIDATION_ERROR:
          'Unable to create ticket. Please fix the highlighted errors.',
...
export function useCustomerTickets() {
  const customerId = useProfileStore(
    (s) => s.profile?.id || s.profile?.user?.id
  );
